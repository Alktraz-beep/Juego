...
/**
Escenario 6 que es el aereopuerto, esta clase permite implementar el tercer escenario en aereopuerto, con el enfrentamiento del segundo jefe 
que es ShubNiggurath en el cual se muestra tambíen 3 botiquines que puede usar el jugador
**/
public class EscenarioAereopuerto extends AnimationTimer{
	private  static final String RUTA_FONDO1=new File("./Images/escenarios/Aeropuerto.png").getAbsolutePath();
	...
	private  static final String RUTA_MMuertoBoss=new File("./Music/muerteBos2.mp3").getAbsolutePath(); //ruta de audio ambiente
	...
	/**
	Constructor del escenario en el aereopuerto que permite inicializar al jefe ShubNiggurath y a los botiquines, ademas de la imagen de escenario y la nueva musica de fondo
	@param grid GridPane que es el grid que se usa para que se pueda añadir o no cosas al escenario
	**/
	public EscenarioAereopuerto(GridPane grid){
		this.grid=grid;
		...
		this.shub=new ShubNiggurath(200,500,140);

	}

	/**
	Metodo sobreescrito que permite hacer el nuevo bucle para la batalla contra el jefe Shub-Niggurath con sus respectivos movimientos para batalla
	...
	**/
	public void handle(long current){
		...
		vidaBoss.setText("Vida Shub-Niggurath: "+shub.getVida());
		puntos.setText("Score : "+Detective.getPuntos());
		botiquines.setText("Botiquines : "+Detective.getNumBotiquines());
		
		this.shub.volar(t);
		this.shub.setOnMouseClicked(new EventHandler<MouseEvent>(){
			@Override
			public void handle(MouseEvent e){
				shub.bajarVida(Detective.getInstance().getDano());
				if(!shub.estaVivo()){
		...
		if(Detective.getInstance().getBoundsInParent().intersects(this.shub.getBoundsInParent()) && this.shub.estaVivo()){
			Detective.getInstance().bajarVida(this.shub.golpe);
			Detective.getInstance().mostrarDano();
			Detective.getInstance().setPuntos(-10);
		}else if(!Detective.getInstance().getBoundsInParent().intersects(this.shub.getBoundsInParent())){
			Detective.getInstance().mostrarNormal();
		}
		if(!this.shub.estaVivo()){
			grid.getChildren().remove(this.shub);
			this.vidaBoss.setText("Vida Shub-Niggurath: MUERTO");
		}
		...
		if(Detective.getInstance().getTranslateX()>NuevoJuego.ANCHO_DE_LA_ESCENA && !this.shub.estaVivo()){
			this.stop();
			...
			mFondo.stop();
			EscenarioMuelle muelle=new EscenarioMuelle(this.grid);
			grid.getChildren().add(muelle.getBackground());
			grid.getChildren().add(muelle.getBotiquinesLbl());
			grid.getChildren().add(muelle.getVida());
			grid.getChildren().add(muelle.getPuntos());
			grid.getChildren().addAll(muelle.getBotiquin());
			grid.getChildren().addAll(muelle.getMejora());
			grid.getChildren().addAll(muelle.getAzathots());
			grid.getChildren().addAll(muelle.getHibs());
			grid.getChildren().add(Detective.getInstance());
			grid.getChildren().addAll(muelle.getCY());
			muelle.start();
		}
	}
	...
	**/
	...
	/**
	Metodo que obtiene la imagen del background que es el aereopuerto
